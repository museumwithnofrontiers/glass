...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('glass.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('glass.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('glass.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
